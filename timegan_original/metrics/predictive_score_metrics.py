...
def predictive_score_metrics(dataX, dataX_hat):
    print('predictive score metrics')
    ...
    def predictor(X, T):
        ...
        with tf.variable_scope("predictor", reuse=tf.AUTO_REUSE) as vs:
            d_cell = tf.nn.rnn_cell.GRUCell(num_units=hidden_dim, activation=tf.nn.tanh)
    ...
    for itt in range(iterations):
        print('training step using predictive data: {}'.format(itt))
        # Batch setting
